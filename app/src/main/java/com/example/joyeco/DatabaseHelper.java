...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public int updatePassword(String email,String newPass){
       // System.out.println("UPDATE "+TABLE_NAME +" SET " + COL_3+ " = '"+newPass+"' WHERE "+COL_2+ " = "+email);
        ContentValues cv = new ContentValues();
        SQLiteDatabase db = getReadableDatabase();
        cv.put(COL_3, newPass);
        System.out.println("humaimam  "+COL_3+" "+newPass+" "+COL_2+" "+email);
        return db.update(TABLE_NAME, cv, COL_2 + "= ?", new String[] {email});
    }
